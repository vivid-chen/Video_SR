...
class VIDEODATA(data.Dataset):
    def __init__(self, args, name='', train=True):
        ...
        self.n_frames_per_train_video = args.n_frames_per_train_video
        self.n_frames_per_test_video = args.n_frames_per_test_video
        print("n_seq:", self.n_seq)
        print("n_colors:", self.n_colors)
        print("n_frames_per_train_video:", args.n_frames_per_train_video)
        print("n_frames_per_test_video:", args.n_frames_per_test_video)
        ...
        if train:
            self.repeat = max(args.test_every // max((self.num_frame // self.args.train_batch_size), 1), 1)
    ...
    def _scan(self):
        ...
        for vid_gt_name, vid_input_name in zip(vid_gt_names, vid_input_names):
            if self.train:
                gt_dir_names = sorted(glob.glob(os.path.join(vid_gt_name, '*')))[:self.args.n_frames_per_train_video] # 截取视频中部分帧进行处理
                input_dir_names = sorted(glob.glob(os.path.join(vid_input_name, '*')))[:self.args.n_frames_per_train_video]
            else:
                gt_dir_names = sorted(glob.glob(os.path.join(vid_gt_name, '*')))[:self.args.n_frames_per_test_video] # 截取视频中部分帧进行处理
                input_dir_names = sorted(glob.glob(os.path.join(vid_input_name, '*')))[:self.args.n_frames_per_test_video]
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.n_seq):
            inputs_concat[:, :, self.n_colors * i:self.n_colors * (i + 1)] = inputs[i]

        gts_concat = np.zeros((gts.shape[1], gts.shape[2], gts.shape[3] * gts.shape[0]))
        for i in range(self.n_seq):
            gts_concat[:, :, self.n_colors * i:self.n_colors * (i + 1)] = gts[i]
        ...
        # patch shape = H, W, color * seq
        for i in range(self.n_seq):
            inputs_list.append(patches[0][:, :, self.n_colors * i:self.n_colors * (i + 1)])
            gts_list.append(patches[1][:, :, self.n_colors * i:self.n_colors * (i + 1)])
        # 及时销毁变量
        del patches
        ...
        inputs_bic_list = []
        for i in range(self.n_seq):
            # cv2.GaussianBlur(inputs_list[i], 15, 2.7, borderType = cv2.BORDER_REPLICATE) # 高斯模糊
        ...
        inputs = np.array(utils.set_channel(*inputs, n_channels=self.n_colors))
        gts = np.array(utils.set_channel(*gts, n_channels=self.n_colors))
        inputs_bics = np.array(utils.set_channel(*inputs_bics, n_channels=self.n_colors))

        input_tensors = utils.np2Tensor(*inputs, rgb_range=self.args.rgb_range, n_colors=self.n_colors)
        gt_tensors = utils.np2Tensor(*gts, rgb_range=self.args.rgb_range, n_colors=self.n_colors)
        input_bic_tensor = utils.np2Tensor(*inputs_bics, rgb_range=self.args.rgb_range, n_colors=self.n_colors)
    ...
    def _get_index(self, idx):
        if self.train:
            ...
            return idx % self.num_frame # 加了个除法，暂时先这么写
