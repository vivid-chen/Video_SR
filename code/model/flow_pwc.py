...
def make_model(args):
    ...
    return nn.DataParallel(Flow_PWC(load_pretrain=True, pretrain_fn=pretrain_fn, device='cuda')) # mul_GPU
...
class Flow_PWC(nn.Module):
    def __init__(self, load_pretrain=False, pretrain_fn='', device='cuda'):
        ...
        self.moduleNetwork = nn.DataParallel(self.moduleNetwork) # mul_GPU

        print("Creating Flow PWC")

        if load_pretrain:
            self.moduleNetwork.load_state_dict(torch.load(pretrain_fn), strict = False)
    ...
    def forward(self, frame_1, frame_2):
        ...
        # warp
        frame_2_warp = self.warp(frame_2, flow)

        return frame_2_warp, flow
...
class Network(torch.nn.Module):
    def __init__(self, device='cuda'):
        ...
        # self.moduleExtractor = torch.nn.DataParallel(self.moduleExtractor)

        self.moduleTwo = Decoder(2)
        # self.moduleTwo = torch.nn.DataParallel(self.moduleTwo)
        self.moduleThr = Decoder(3)
        # self.moduleThr = torch.nn.DataParallel(self.moduleThr)
        self.moduleFou = Decoder(4)
        # self.moduleFou = torch.nn.DataParallel(self.moduleFou)
        self.moduleFiv = Decoder(5)
        # self.moduleFiv = torch.nn.DataParallel(self.moduleFiv)
        self.moduleSix = Decoder(6)
        # self.moduleSix = torch.nn.DataParallel(self.moduleSix)

        self.moduleRefiner = Refiner()
        # self.moduleRefiner = torch.nn.DataParallel(self.moduleRefiner)
