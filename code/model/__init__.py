...
class Model(nn.Module):
    def __init__(self, args, ckp):
        ...
        self.save_middle_models = args.save_middle_models # 是否保存中间结果

        module = import_module('model.' + args.model.lower()) # lower是全部变成小写 RCAN
        self.model = module.make_model(args).to(self.device)
        a = range(args.n_GPUs)
        if not args.cpu and args.n_GPUs > 1:
            self.model = nn.DataParallel(self.model, range(args.n_GPUs)) # 多GPU设置并行
            # self.model = nn.DataParallel(self.model, device_ids = [0,1]) # 多GPU设置并行
    ...
    def load(self, apath, pre_train='.', resume=False, cpu=False):  #
        ...
        if pre_train != '.':
            ...
            self.get_model().load_state_dict(
                torch.load(self.args.test_model_path, **kwargs),
                strict=False
            )
        else:
            pass
