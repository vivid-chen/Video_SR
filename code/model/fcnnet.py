...
class FCNNET(nn.Module):
    def __init__(self, args):
        ...
        if self.fcn_number == 1:
            ...
            self.lineLayer1 = nn.DataParallel(self.lineLayer1) # mul_GPU

            self.reluLayer = nn.ReLU()
            self.reluLayer = nn.DataParallel(self.reluLayer) # mul_GPU

            self.lineLayer2 = nn.Linear(self.num_hidden, self.num_output_channels, bias=True)
            self.lineLayer2 = nn.DataParallel(self.lineLayer2) # mul_GPU
